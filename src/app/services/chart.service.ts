...
export class ChartService {
    ...
    constructor(private conjugationService: ConjugationService, private http: HttpClient, public snackBar: MatSnackBar, private verbService: VerbService, private pronounService: PronounService, private affixService: AffixService) {

    }

    returnAllData() {
        return this.http.get(environment.base + '/static/web/assets/data.json').pipe(
            map(data => {
                data = data
    ...
    createChart(tableargs, order, depth) {
        return this.conjugationService.conjugate(tableargs)
            .pipe(
                map(res => this.createChartData(res, order, depth))
            )
    }
