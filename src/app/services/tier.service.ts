...
import { Response, ResponseObject, ResponseValue } from '../models'
...
export class TierService {
    ...
    constructor(private affixService: AffixService, private pronounService: PronounService,
        private verbService: VerbService, private http: HttpClient,
    ...
    createTiers(conjugations: Response[]) {
        return conjugations.map(conjugation => {
            ...
            return {
                'main': this.createTier(values,
                    { 'keys': ['value'], 'seperator': '' },
                    { 'keys': ['value'], 'seperator': '' },
                    { 'keys': ['value'], 'seperator': '' }),

                'secondary': this.createTier(values,
                    { 'keys': [' root'], 'seperator': '' },
                    {
                        'keys': [
                            'agent',
                ...
                'ternary': this.createTier(values,
                    { 'keys': ['gloss'], 'seperator': '' },
                    {
                        'keys': [
                            'gloss',
    ...
    createTier(conjugation, verb_key_obj, pn_key_obj, aff_key_obj) {
        ...
        const verb_value = verb_key_obj['keys'].map(key => {
        ...
        affixes.forEach((affix) => {
            if (affix.value != null && affix.value !== '') {
                const aff_value = aff_key_obj['keys'].map(key => {
                    // if the key exists in the conjugation provided, return the value
